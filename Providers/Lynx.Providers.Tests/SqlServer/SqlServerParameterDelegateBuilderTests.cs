...
public class SqlServerParameterDelegateBuilderTests : ParameterDelegateBuilderTestsBase
{
    [Theory]
    [MemberData(nameof(GetEntityTypes))]
    public void BuildAddDelegate(Type entityType)
    {
        using var harness = new SqlServerTestHarness([nameof(BuildAddDelegate), entityType.Name]);
        using var context = harness.CreateContext();

        var entity = EntityInfoFactoryTests.CreateRootEntity(entityType, context.Model);
        var action = AddParameterDelegateBuilder<SqlCommand, SqlServerProviderDelegateBuilder>.Build(entity);
    ...
    {
        using var harness = new SqlServerTestHarness([nameof(BuildCitySetParametersDelegate)]);
        using var context = harness.CreateContext();

        var entity = EntityInfoFactory.CreateRoot<City>(context.Model);

        var addParameters = AddParameterDelegateBuilder<SqlCommand, SqlServerProviderDelegateBuilder>.Build(entity);

        var command = new SqlCommand();
        addParameters(command);

        var setParameters = SetParameterValueDelegateBuilder<SqlCommand, SqlServerProviderDelegateBuilder, City>.Build(entity);
    ...
    {
        using var harness = new SqlServerTestHarness([nameof(BuildCustomerSetParametersDelegate)]);
        using var context = harness.CreateContext();
        ...
        var setParameters = SetParameterValueDelegateBuilder<SqlCommand, SqlServerProviderDelegateBuilder, Customer>.Build(entity);
    ...
    {
        using var harness = new SqlServerTestHarness([nameof(BuildConverterEntitySetParametersDelegate)]);
        using var context = harness.CreateContext();
        ...
        var setParameters = SetParameterValueDelegateBuilder<SqlCommand, SqlServerProviderDelegateBuilder, ConverterEntity>.Build(entity);
    ...
    private static string? SerializeJson(object? value) => JsonHelpers.SerializeJson(value);
}